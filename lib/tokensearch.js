...
Tokensearch.prototype._isScalarValue = function(item) {
...
Tokensearch.prototype._prepareSearchTokens = function() {
  ...
  this.collection.forEach(entry => {
    var tmp = [];
    this.collectionKeys.forEach(key => {
      let prop = entry.item[key];
      if (prop) {
        let str = prop.toString().trim().toLowerCase();
        str.split(this.delimiter).forEach(function(e) {
          tmp = tmp.concat(e);
        });
      }
    });
